  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center px-6 overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-5">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-accent-yellow rounded-full blur-[120px]" />
      </div>
      ...
      <div className="relative z-10 max-w-6xl w-full">
        ...
        <motion.h1
          ...
          initial="hidden"
          animate="visible"
          className="font-display text-7xl sm:text-8xl md:text-9xl lg:text-[12rem] font-bold leading-none mb-6 md:mb-8 flex"
        >
          {letters.map((letter, index) => (
            <motion.span
              key={index}
              variants={child}
              className={`hover:text-accent-yellow transition-colors duration-300 ${letter === ' ' ? 'w-4 md:w-6 lg:w-8' : ''}`}
            >
              {letter === ' ' ? '\u00A0' : letter}
            </motion.span>
          ))}
        ...
        >
          <a
            ...
            className="px-6 py-3 bg-dark-secondary hover:bg-accent-yellow text-light-primary hover:text-dark-primary rounded-lg transition-all duration-300"
          >
            GitHub
          </a>
          <a
      ...
      <motion.button
        ...
        className="absolute bottom-8 left-1/2 -translate-x-1/2 text-light-secondary hover:text-accent-yellow transition-colors duration-300"
